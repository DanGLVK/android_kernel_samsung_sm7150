...
#define ATOMIC_FETCH_OP(name, mb, op, asm_op, cl...)                           \
  static inline int atomic_fetch_##op##name(int i, atomic_t *v) {              \
    ...
    register atomic_t *x1 asm("x1") = v;                                       \
                                                                               \
    asm volatile(                                                              \
        __LSE_PREAMBLE ARM64_LSE_ATOMIC_INSN(/* LL/SC */                       \
...
static inline void atomic_and(int i, atomic_t *v) {
  ...
  asm volatile(__LSE_PREAMBLE ARM64_LSE_ATOMIC_INSN(
                   /* LL/SC */
                   __LL_SC_ATOMIC(and) __nops(2),
                   /* LSE atomics */
                   "	prfm	pstl1strm, %[v]\n"
                   "	mvn	%w[i], %w[i]\n"
                   "	stclr	%w[i], %[v]")
...
static inline long atomic64_dec_if_positive(atomic64_t *v) {
  ...
  asm volatile(__LSE_PREAMBLE ARM64_LSE_ATOMIC_INSN(
                   /* LL/SC */
                   __LL_SC_ATOMIC64(dec_if_positive) __nops(7),
                   /* LSE atomics */
                   "	prfm	pstl1strm, %[v]\n"
                   "1:	ldr	x30, %[v]\n"
                   "	subs	%[ret], x30, #1\n"
...
#define __CMPXCHG_CASE(w, sfx, name, sz, mb, cl...)                            \
  static inline u##sz __cmpxchg_case_##name##sz(                               \
      volatile void *ptr, unsigned long old, u##sz new) {                      \
...
#define __CMPXCHG_DBL(name, mb, cl...)                                         \
  static inline long __cmpxchg_double##name(                                   \
      unsigned long old1, unsigned long old2, unsigned long new1,              \
      unsigned long new2, volatile void *ptr) {                                \
